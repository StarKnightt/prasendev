...
Most MCP servers are just a JSON config with a command to run. That's it.

Under the hood it looks like this. The app you use runs one MCP client per server, and each server exposes a few tools the model can call:

![What's inside MCP: the AI app runs one MCP client per server, each server exposes tools like navigate, create PR or run a query, and talks to the real thing](/blog/mcp-architecture.webp) No Docker containers, no Kubernetes clusters, no 47-step deployment pipeline.
