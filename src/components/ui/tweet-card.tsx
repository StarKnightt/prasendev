...
export const TweetHeader = ({ tweet }: { tweet: EnrichedTweet }) => (
  <div className="flex flex-row items-start justify-between tracking-normal">
    ...
    <div className="pointer-events-auto group/icon">
      <Twitter className="text-muted-foreground group-hover/icon:text-blue-500 group-hover/icon:scale-110 size-5 items-start transition-all duration-300 ease-out" />
    </div>
  </div>
)
...
export const TweetCard = async ({
  ...
  return (
    <Suspense fallback={fallback}>
      <MagicTweet tweet={tweet} className={className} />
    </Suspense>
  )
}
